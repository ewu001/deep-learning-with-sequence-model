...
if [ "$1" = "train" ]; then
	CUDA_VISIBLE_DEVICES=0 python run.py train --train-src=./en_es_data/train.en --train-tgt=./en_es_data/train.es --dev-src=./en_es_data/dev.en --dev-tgt=./en_es_data/dev.es --vocab=vocab.json --max-epoch=5 --pretrain-model=./model_output/model.bin --batch-size=4 --cuda
elif [ "$1" = "test" ]; then
