...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(word_embed_size, hidden_size, 1, True, False, dropout_rate, True)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # Tensor: (src_len, b, max_word_src)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        init_decoder_hidden_concat = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(init_decoder_hidden_concat)

        init_decoder_cell_concat = torch.cat((last_cell[0], last_cell[1]), dim=1)
        init_decoder_cell = self.c_projection(init_decoder_cell_concat)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        y_tgt = self.model_embeddings_target(target_padded)
        for y_t in torch.split(y_tgt, 1):
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # includes target embedding input at t, 
        # concats with previous combined output of previous attention vector and previous decoder hidden
        ...
        # Obtain attention score distribution, and then attention vector
